...
audio_tfrecord = '/media/admin1/Windows/MAPS_TFRECORD/maps_config2_test.tfrecord'
spec_tfrecord = '/home/admin1/data/tfrecord/maps/maps_config2_test_spec.tfrecord'
'''
    ...
    with tf.Session() as sess:
        ...
        try:
            ...
            while True:
                ...
                '''
                spec = feature.spec.flatten()
                length = feature.length.flatten()
                sequence_id = feature.sequence_id
                spectrogram_hash = feature.spectrogram_hash.flatten()

                labels = label.labels.flatten()
                label_weights = label.label_weights.flatten()
                onsets = label.onsets.flatten()
                offsets = label.offsets.flatten()
                velocities = label.velocities.flatten()
                note_sequence = label.note_sequence
