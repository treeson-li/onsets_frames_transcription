...
def transformer_layer(inputs, labels, params, is_training=True):
  """Create a Sparse Transformaer Network layer."""
  return transformer.sparse_transformer(inputs, labels, params, is_training)
...
def acoustic_model(inputs, hparams, lstm_units, lengths, labels=None, is_training=True, use_transformer=False):
  ...
  if use_transformer:
    return transformer_layer(conv_output, labels, hparams, is_training)
  ...
  with slim.arg_scope([slim.batch_norm, slim.dropout], is_training=is_training):
    ...
    with tf.variable_scope('frame'):
      ...
      if hparams.use_transformer:
        outputs = transformer_layer(combined_probs, frame_labels, hparams, is_training=is_training)
