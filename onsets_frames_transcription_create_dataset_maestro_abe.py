...
tf.app.flags.DEFINE_string('input_dir', '/media/ps/Windows/Data/maestro-v1.0.0/',
...
def generate_train_set(jsonpath):
  ...
  for item in fileJson:
    if(item["split"]=='train'):
      ...
      canonical_composer = item["canonical_composer"]
      split = item["split"]
      midi_filename = item["midi_filename"]
      canonical_title = item["canonical_title"]
      wav_file = os.path.join(FLAGS.input_dir, audio_filename)
      mid_file = os.path.join(FLAGS.input_dir, midi_filename)
      # print(wav_file)
      # print(mid_file)
      # print(split)
      train_file_pairs.append((wav_file, mid_file))
  ...
  train_output_name = os.path.join(FLAGS.output_dir,
                                    'maestro_new_config2_train.tfrecord') 
...
def generate_test_set(jsonpath):
  ...
  test_output_name = os.path.join(FLAGS.output_dir,
                                  'maestro_new_config2_test.tfrecord')
...
def generate_validation_set(jsonpath):
  """Generate the test TFRecord."""
  test_file_pairs = []
  # for directory in test_dirs:
  #   path = os.path.join(FLAGS.input_dir, directory)
  ...
  # exit()
  test_output_name = os.path.join(FLAGS.output_dir,
                                  'maestro_new_config2_validation.tfrecord')
...
def main(unused_argv):
  ...
  path_json = 'maestro-v1.0.0.json'
  # generate_test_set(path_json)
  generate_train_set(path_json)
  # generate_validation_set(path_json)
