...
def get_2d_attn_mask(n, attn_mode, local_attn_ctx=None):
    if attn_mode == 'all':
        b = tf.matrix_band_part(tf.ones([n, n]), -1, 0)
    elif attn_mode == 'local':
        bandwidth = local_attn_ctx
        ...
        raise ValueError('Not yet implemented')
    print('-'*50, 'mask=', b, 'len(mask)=', len(b.shape))
    return b
...
def get_blocksparse_obj(n_ctx, n_heads, attn_mode, blocksize=32, local_attn_ctx=None, num_verts=4, vertsize=1):
    ...
    layout = get_2d_attn_mask(n_bctx, attn_mode, local_attn_ctx)
    print('='*50, 'len(layout)=', len(layout.shape))
