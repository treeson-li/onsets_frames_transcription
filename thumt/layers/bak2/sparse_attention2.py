...
def blocksparse_attention_impl(q, k, v, heads, attn_mode, local_attn_ctx=None,
                               blocksize=32, keep_prob=None, num_verts=None, vertsize=None):
...
def get_blocksparse_obj(n_ctx, n_heads, attn_mode, blocksize=32, local_attn_ctx=None, num_verts=4, vertsize=1):
    ...
    layout = tf.ones([n_bctx, n_bctx], dtype=tf.bool)
    ...
    if attn_mode == 'fixed':
        ...
        indices = tf.reshape(indices, [-1, vertsize])
        if num_verts == 1:
            layout = tf.zeros([n_bctx, n_bctx], dtype=tf.bool)
            for idx in indices[0]:
                layout[:, idx::stride] = 1
            for q_idx in tf.range(n_bctx):
            ...
            layout = tf.convert_to_tensor(np.array(layouts))
    else:
        def cond(i, layout):
            return tf.less(i, n_bctx*n_bctx)

        def body(i, layout):
            q_idx = i // n_bctx
            k_idx = i % n_bctx
        
            layout[q_idx, k_idx] = tf.cond(tf.greater(k_idx, q_idx), lambda: False, lambda: True)
            layout[q_idx, k_idx] = tf.cond(tf.greater(extra_diagonals, 0) and tf.less(k_idx + extra_diagonals, q_idx), lambda: False, lambda: True)            
            if block_chunks is not None:
                layout[q_idx, k_idx] = False
                offset = q_idx % block_chunks
                if k_idx + offset >= q_idx and k_idx <= q_idx:
                    layout[q_idx, k_idx] = True
            i = i + 1
            return i, layout

        i = tf.constant(0, dtype=tf.int32)
        i, layout = tf.while_loop(cond, body, [i, layout])
...
def get_callback(attn_mode, local_attn_ctx=None):
    ...
    def cb(blk_shape, head_idx, qry_idx, key_idx, blk_idx):
        mask = tf.ones(blk_shape, dtype=tf.bool)

        # on the diagonal blocks mask out the upper diagonal
        if qry_idx == key_idx:
            for q in tf.range(blk_shape[0]):
                for k in tf.range(blk_shape[1]):
                    if k > q:
                        mask[q, k] = 0
