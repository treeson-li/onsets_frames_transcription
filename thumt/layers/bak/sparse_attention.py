...
def get_blocksparse_obj(n_ctx, n_heads, attn_mode, blocksize=32, local_attn_ctx=None, num_verts=4, vertsize=1):
    ...
    if attn_mode == 'fixed':
        ...
        idxs = tf.where(tf.zeros([n_bctx, n_bctx]))
        idxs = tf.reshape(idxs, [-1, 2])
        def cond(i):
            return tf.less(i, tf.shape(idxs)[0])

        def deal_layout(i):
            q_idx = idxs[i][0]
            k_idx = idxs[i][1]
            layout[q_idx, k_idx] = tf.cond(tf.greater(k_idx, q_idx), lambda: 0, lambda:layout[q_idx, k_idx])
        ...
        process = tf.while_loop(cond, deal_layout, [i])
        '''
        for q_idx, k_idx in idxs:
            if k_idx > q_idx:
                layout[q_idx, k_idx] = 0
            if extra_diagonals and k_idx + extra_diagonals < q_idx:
                layout[q_idx, k_idx] = 0
